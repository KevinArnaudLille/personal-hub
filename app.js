...
function render() {
  ...
  document.getElementById('cadrage-list').innerHTML = hub.cadrages.length
    ? hub.cadrages.map(cadrage => `<a id="${esc(cadrage.id)}" class="cadrage-link" href="${esc(cadrage.url)}"><span><strong>${esc(cadrage.title)} <code class="cadrage-id">${esc(cadrage.id)}</code></strong><small>${esc(cadrage.summary)}</small></span><span aria-hidden="true">→</span></a>`).join('')
